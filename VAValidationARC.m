...
static inline int hCheck( int argc, startup_call_t *theCall, id * dataPtr, id b)
{
    ...
    NSString * bidStr = [[NSString alloc] initWithBytes: (bundle_id->buf + 2) length: (bundle_id->size - 2) encoding: NSUTF8StringEncoding];
    ...
    NSString * dvStr = [[NSString alloc] initWithBytes: (bundle_version->buf + 2) length: (bundle_version->size - 2) encoding: NSUTF8StringEncoding];
...
+(int)v:(NSBundle*)b{
	...
	OSStatus existence = SecStaticCodeCreateWithPath((__bridge CFURLRef)[b bundleURL], kSecCSDefaultFlags, &staticCode);
    OSStatus validity = -1;    
    if (existence == noErr){
        ...
        if (SecStaticCodeCreateWithPath(url, kSecCSDefaultFlags, &codeRef) == noErr) {
            if (SecStaticCodeCheckValidity(codeRef, kSecCSBasicValidateOnly, NULL) == errSecSuccess) {
                ...
                if (SecCodeCopySigningInformation(codeRef, kSecCSSigningInformation, &api) == noErr) {  
                    NSArray *certs = [(__bridge NSDictionary*)api objectForKey:(NSString*)kSecCodeInfoCertificates];
                    for (id cert in certs) {
                        CFStringRef commonName;
                        SecCertificateCopyCommonName((__bridge SecCertificateRef)cert, &commonName);
                        if (commonName) {
                            [ret addObject:[NSString stringWithString:(__bridge NSString*)commonName]];
