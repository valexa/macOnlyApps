...
- (id)initWithAnimationCurve:(NSAnimationCurve)animationCurve animatingContentView:(NSView *)aView
{
    self = [super initWithDuration: NSWindowFlippingAnimationNullDuration animationCurve: animationCurve];
    if (!self) return nil;
...
- (void)setInitialWindow:(NSWindow *)anInitialWindow finalWindow:(NSWindow *)aFinalWindow andFlippingDirection:(BOOL)isForward
{
    ...
    [flippingContainerWindow setFrame: flippingContainerWindowFrame display: NO];
    ...
    [windowViewSuperview cacheDisplayInRect: windowViewSuperViewFrame toBitmapImageRep: windowBitmapRepresentation];
    ...
    //NSDisableScreenUpdates();
    {
        ...
        [flippingContainerWindow orderWindow: NSWindowBelow relativeTo: [finalWindow windowNumber]];
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    // Draw
    NSRect originalBounds = NSMakeRect(-originalFrame.origin.x, -originalFrame.origin.y, NSWidth([self bounds]), NSHeight([self bounds]));
    
    [windowShadow set];
    [outputImage drawInRect: [self bounds] fromRect: originalBounds operation: NSCompositeSourceOver fraction: 1];
...
+ (NSWindow *)flippingContainerWindow
{
    if (!flippingContainerWindow)
    {
        ...
        NSWindowFlippingView *flippingView = [[NSWindowFlippingView alloc] initWithFrame: NSWindowFlippingContainerWindowDefaultContentRect andOriginalFrame: NSWindowFlippingContainerWindowDefaultOriginalFrame];
...
- (void)flipToShowWindow:(NSWindow *)window forward:(BOOL)forward
{
    ...
    [(NSWindowFlippingView *) [[NSWindow flippingContainerWindow] contentView] setInitialWindow:self finalWindow:window andFlippingDirection: forward];
}
